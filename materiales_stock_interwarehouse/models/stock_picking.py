...
class StockPicking(models.Model):
    ...
    def button_request(self):
        for picking in self:
            ...
            admins = picking.env['res.users'].search([('location_ids', 'in', [picking.location_id.id])])
            if admins:
                for admin in admins:
    ...
    def _compute_show_validate(self):
        for picking in self:
            ...
            elif picking.picking_type_id.sequence_code == 'INT' and (picking.state == 'draft' or (picking.state == 'to_approve' and picking.location_id.id not in self.env.user.location_ids.ids)):
                picking.show_validate = False
